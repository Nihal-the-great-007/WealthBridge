# Let's generate the updated templates/index.html v3
# Updates:
# - Added print-color-adjust: exact to print stylesheet to preserve visual theme, colors, and repeating background pattern in PDF export.
# - Added page-break-after: always to separate dashboard sections cleanly into a 3-page executive PDF report.
# - Implemented beforeprint and afterprint event hooks in JS to temporarily expand all hidden panels and force Chart.js to resize/redraw, ensuring charts print perfectly.
    ...
    @media print {
      * {
      ...
      body {
        ...
        background-image: url('/background-pattern.jpg') !important;
      ...
      .right-panel, .panel, .chart-panel, .metric-card, .housing-card {
        ...
        border: 1px solid rgba(0, 131, 176, 0.20) !important;
      ...
      .data-table th, .data-table td {
        border: 1px solid rgba(0, 131, 176, 0.15) !important;
      ...
      .donut-legend-item {
        border: 1px solid rgba(0, 131, 176, 0.10) !important;
      ...
      .income-card {
        border: 1px solid rgba(0, 131, 176, 0.2) !important;
        background: rgba(255, 255, 255, 0.95) !important;
      }
    }
  </style>
...
<body class="light-mode">
  ...
  <script>
    ...
    /* ═══════════════════════════════════════════════════════════════════════════
       PRINT PREVIEW SYSTEM - Ensure all hidden charts resize/redraw perfectly
       ═══════════════════════════════════════════════════════════════════════════ */
    window.addEventListener('beforeprint', () => {
      // Force all tab panels to be display: block temporarily so charts calculate sizes
      document.querySelectorAll('.tab-panel').forEach(p => {
        p.style.display = 'block';
      });
      // Force Chart.js instances to resize and redraw synchronously
    ...
    window.addEventListener('afterprint', () => {
      // Restore tab panel visibility states based on active tab button
      ...
      // Re-trigger resize on visible charts
      Object.values(_charts).forEach(c => {
        if (c) c.resize();
      });
    });
...
print("Successfully rewrote templates/index.html with print preview and background-graphics fixes!")
