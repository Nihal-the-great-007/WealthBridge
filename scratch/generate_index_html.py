# Let's generate the updated templates/index.html
# We will create a clean visual system, resolve duplicate loaders, build the Guilloche canvas, unhide the housing analysis, and fix data export.
...
<head>
  ...
  <style>
    ...
    body.light-mode {
      --bg: #F4F6F9;
      --panel: #FFFFFF;
      --panel-border: rgba(122, 63, 168, 0.15);
      --panel-hover: rgba(122, 63, 168, 0.06);
      --purple: #7A3FA8; /* Elegant Deep Violet */
      --purple-dim: rgba(122, 63, 168, 0.10);
      --purple-glow: rgba(122, 63, 168, 0.20);
      --magenta: #D81B60;
      ...
      --amber: #EF6C00;
      --text-primary: #1E1E2F;
      --text-secondary: #5F5F75;
      --text-muted: #8E8E9F;
      --topbar-bg: rgba(244, 246, 249, 0.85);
      --input-bg: rgba(0, 0, 0, 0.03);
      --income-row-bg: rgba(0, 0, 0, 0.02);
      --table-thead-bg: rgba(244, 246, 249, 0.98);
      --accent-gradient: linear-gradient(135deg, #7A3FA8, #9C27B0);
    }
    ...
    *::after {
    ...
    body.light-mode .console-section-card:hover {
      ...
      background: rgba(122, 63, 168, 0.02);
    }
...
<body>
  ...
  <header class="topbar">
    ...
    <div class="topbar-right">
      ...
      <button id="theme-toggle" class="action-btn theme-toggle-btn" onclick="toggleTheme()" aria-label="Toggle bright mode" title="Toggle bright mode">
        ...
        <span class="theme-toggle-text">Bright Mode</span>
      </button>
    ...
    <div class="main-content">
      <div class="tab1-grid">
        ...
        <aside class="control-console">
          <div class="panel" style="padding: 18px; display: flex; flex-direction: column; gap: 16px; background: rgba(22, 22, 31, 0.4); backdrop-filter: blur(12px); border-color: var(--panel-border);">
            ...
            <div class="chart-header">
              ...
              <div class="chart-legend">
                <div class="legend-item">
                  <div class="legend-dot" style="background:#7C3AED;"></div> P10 Pessimistic
                </div>
                <div class="legend-item">
                  <div class="legend-dot" style="background:#9D4EDD;"></div> P50 Expected
                </div>
                <div class="legend-item">
                  <div class="legend-dot" style="background:#C77DFF;"></div> P90 Optimistic
                </div>
              </div>
            </div>
  ...
  <script>
    ...
    Chart.defaults.plugins.tooltip.bodyColor = '#A0A0BA';
    ...
    function updateChartThemeDefaults(isLight) {
      Chart.defaults.color = isLight ? '#4E4E62' : '#A0A0BA';
      ...
      Chart.defaults.plugins.tooltip.bodyColor = isLight ? '#4E4E62' : '#A0A0BA';
    }
    ...
    function toggleTheme() {
      ...
      const tickColor = isLight ? '#4E4E62' : '#A0A0BA';
      const labelColor = isLight ? '#4E4E62' : '#A0A0BA';
          ...
          datasets: [
            {
              ...
              backgroundColor: () => {
                ...
                g.addColorStop(0, 'rgba(199,125,255,0.15)');
                g.addColorStop(1, 'rgba(199,125,255,0.01)');
                return g;
              },
            },
            {
              ...
              backgroundColor: () => {
                const g = ctx.createLinearGradient(0, 0, 0, 300);
                g.addColorStop(0, 'rgba(157,78,221,0.12)');
                g.addColorStop(1, 'rgba(157,78,221,0.01)');
                return g;
              },
            },
            {
        ...
        options: {
          ...
          scales: {
            x: {
              ...
              ticks: {
                color: isLight ? '#4E4E62' : '#606075', maxTicksLimit: 12,
        ...
        options: {
          ...
          plugins: {
            legend: {
              display: true,
              position: 'top',
              labels: { boxWidth: 10, padding: 16, color: isLight ? '#4E4E62' : '#A0A0BA', font: { size: 11 } }
            },
            tooltip: {
              callbacks: {
          ...
          scales: {
            x: {
              grid: { display: false },
              ticks: { color: isLight ? '#4E4E62' : '#606075', maxTicksLimit: 10, font: { size: 10 } }
            },
            y: {
              grid: { color: isLight ? 'rgba(122,63,168,0.06)' : 'rgba(157,78,221,0.06)' },
              ticks: { color: isLight ? '#4E4E62' : '#A0A0BA', callback: v => fmtINR(v, true) }
            }
          }
        }
      });
        ...
        options: {
          ...
          scales: {
            x: {
              ...
              ticks: { color: isLight ? '#4E4E62' : '#606075', maxTicksLimit: 12, font: { size: 10 } }
            },
            y: {
    ...
    /* ═══════════════════════════════════════════════════════════════════════════
       ...
       ═══════════════════════════════════════════════════════════════════════════ */
    function exportData() {
      // Export current user calculations (_data) instead of baseData, fall back to baseData
    ...
    /* ═══════════════════════════════════════════════════════════════════════════
       BOOTSTRAP ON LOAD (Merged & Fixed startup screen loading workflow)
       ═══════════════════════════════════════════════════════════════════════════ */
    document.addEventListener('DOMContentLoaded', async () => {
      // 1. Initialize Canvas-based Guilloche Wave Animation
      ...
      if (canvas) {
        ...
        const drawGuilloche = () => {
          ...
          // Render beautiful linear dark background gradient
          const bgGrad = ctx.createLinearGradient(0, 0, 0, canvas.height);
          bgGrad.addColorStop(0, '#06060a');
          bgGrad.addColorStop(1, '#0e0f18');
          ...
          // Draw 6 interlacing mathematical waves
          for (let w = 0; w < 6; w++) {
            ctx.beginPath();
            ctx.lineWidth = w === 0 ? 1.5 : 0.8;
            
            // Soft neon linear gradients
            const waveGrad = ctx.createLinearGradient(0, 0, canvas.width, 0);
            if (w % 3 === 0) {
              waveGrad.addColorStop(0, 'rgba(0, 180, 219, 0)');
              waveGrad.addColorStop(0.5, 'rgba(0, 180, 219, 0.25)');
              waveGrad.addColorStop(1, 'rgba(0, 180, 219, 0)');
            } else if (w % 3 === 1) {
              waveGrad.addColorStop(0, 'rgba(157, 78, 221, 0)');
              waveGrad.addColorStop(0.5, 'rgba(157, 78, 221, 0.28)');
              waveGrad.addColorStop(1, 'rgba(157, 78, 221, 0)');
            } else {
              waveGrad.addColorStop(0, 'rgba(224, 30, 55, 0)');
              waveGrad.addColorStop(0.5, 'rgba(224, 30, 55, 0.22)');
              waveGrad.addColorStop(1, 'rgba(224, 30, 55, 0)');
            }
            
            ctx.strokeStyle = waveGrad;
            
            for (let x = 0; x < canvas.width; x += 3) {
              // Mathematical Guilloche fine-line formulas
              const carrier = Math.sin(x * 0.002 + timeVal * 0.4 + w * 0.2);
              const mod1 = Math.sin(x * 0.015 + timeVal * 1.2 + w * Math.PI / 3);
              const mod2 = Math.cos(x * 0.035 - timeVal * 1.8 + w * Math.PI / 6);
              
              const y = cy + carrier * 160 + mod1 * 25 + mod2 * 10;
      ...
      try {
        ...
        // Ensure at least 3 seconds is shown for visual enjoyment
...
# Let's write this directly as a clean utf-8 file to templates/index.html
with open("templates/index.html", "w", encoding="utf-8") as f:
    f.write(content)

print("Successfully rewrote templates/index.html!")
